...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Show warnings if data files are empty
    warnings = []
    if not _PROXY_LIST:
        warnings.append("⚠️ No proxies loaded. Checks will use direct connection.")
    if not _SITE_LIST:
        warnings.append("⚠️ No sites loaded. Use `/sh CC|MM|YYYY|CVV https://site.com`")
    if not _CC_LIST:
        warnings.append("⚠️ No cards loaded. Upload `cc.txt` or use `/sh` with a card inline.")

    warning_text = "\n".join(warnings)
    if warning_text:
        warning_text = f"**─── Data Status ───**\n{warning_text}\n\n"
    ...
    await update.message.reply_text(
        warning_text + config.ABOUT_TEXT,
        reply_markup=keyboard,
        parse_mode="Markdown"
    )
...
async def cmd_sh(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Determine site URL: from command args, or sites.txt, or error
    site_url = None
    for arg in context.args:
        if arg.startswith("http://") or arg.startswith("https://") or "." in arg.replace("/", ""):
            site_url = arg
            break

    if not site_url and _SITE_LIST:
        site_url = _SITE_LIST[0]
    elif not site_url:
        await msg.edit_text(
            "❌ **No site specified.**\n\n"
            "Usage: `/sh CC|MM|YYYY|CVV https://site.com`\n"
            "Or add sites to `sites.txt` for default.",
            parse_mode="Markdown"
        )
        return
...
async def cmd_mst(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Mass check: /mst [url] (reply to .txt file or paste cards)"""
    if not await _is_member(update, context):
        return
    if not _check_rate_limit(update.effective_user.id):
        await update.message.reply_text("⏳ **Rate limit.** Please wait.", parse_mode="Markdown")
    ...
    if not site_url and _SITE_LIST:
        ...
        await update.message.reply_text(
            "❌ **No site specified.**\n\n"
            "Usage: `/mst https://site.com` (reply to .txt file)\n"
            "Or add sites to `sites.txt` for a default target.",
            parse_mode="Markdown"
        )
        return

    # ── Parse cards from file or message ──────────────────────────────────
    cards = []
    ...
    # Cards pasted in message body
    if not cards and non_site_args:
        body = " ".join(non_site_args).strip()
        if "\n" in body:
    ...
    if not cards:
        await update.message.reply_text(
            "Usage: `/mst https://site.com` (reply to a `.txt` file)\n"
            "Or paste cards directly after the URL.\n\n"
    ...
    progress_msg = await update.message.reply_text(
        f"⚡ **Mass Check Progress** — `{site_url}`\n\n"
        f"✅ 0/{len(cards)} (0%)\n"
        f"⏱ Starting...",
        parse_mode="Markdown",
    ...
    # Run in background
    asyncio.create_task(_run_mass_check(update, context, cards, site_url, progress_msg, user_id))


async def _run_mass_check(update, context, cards, site_url, progress_msg, user_id):
    ...
    async def worker(cc_str):
        ...
        proxy_str = ROTATOR.next() or get_random_proxy()
        ok, msg, gw, price, cur = await process_card(
            p['cc'], p['mes'], p['ano'], p['cvv'], site_url, None, proxy_str
        )
        resp = extract_clean_response(msg)
...
def main():
    ...
    # Create data files if needed (empty)
    for fname in ["proxies.txt", "cc.txt", "sites.txt"]:
        path = os.path.join(config.BASE_DIR, fname)
        if not os.path.exists(path):
            with open(path, "w") as f:
                f.write(f"# ShopifyChk — add your data below\n")

    # Reload to pick up any existing real data
    if not _PROXY_LIST:
        print("  [!] proxies.txt is empty — checks will use direct connection (no proxy)")
    if not _SITE_LIST:
        print("  [!] sites.txt is empty — users must provide a site URL manually")
    if not _CC_LIST:
        print("  [!] cc.txt is empty — no cards loaded (users can upload via Telegram)")
